...
export default function ClientApp() {
  ...
  // 📡 Realtime: Sincronización y Auto-scroll
  useEffect(() => {
  ...
  // 🛠️ FUNCIÓN CORREGIDA: enviarMensaje (Faltaba en tu código)
  const enviarMensaje = async (e) => {
    e.preventDefault();
    if (!nuevoMsj.trim() || !viajeActivo) return;

    const msgAEnviar = nuevoMsj;
    setNuevoMsj(''); // Limpiar input inmediatamente (UX)
      ...
      {!viajeActivo ? (
        <div className="w-full space-y-4">
          <form onSubmit={revisarTarifa} className="w-full space-y-4 animate-in fade-in duration-500">
            <div className="bg-zinc-900/50 p-6 rounded-[32px] border border-white/5 space-y-6 shadow-2xl">
            ...
            <button type="submit" className="w-full bg-white text-black py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-[#39FF14] transition-all active:scale-95">
              Ver Precio
            </button>
          </form>
          ...
          {mostrandoPrecio && (
            <div className="fixed bottom-10 left-6 right-6 z-50 animate-in slide-in-from-bottom-10">
              <div className="bg-zinc-900 border border-[#39FF14]/30 p-6 rounded-[32px] shadow-[0_0_50px_rgba(0,0,0,1)]">
        ...
        <div className="w-full space-y-6 animate-in zoom-in duration-300">
          <div className="bg-zinc-900 border-2 border-[#39FF14] p-8 rounded-[40px] text-center shadow-[0_0_30px_rgba(57,255,20,0.1)]">
            <div className="text-5xl mb-6">{viajeActivo.estado_via_viaje === 'aceptado' ? '🚕' : '📡'}</div>
            <h2 className="text-2xl font-black uppercase italic tracking-tighter">
              {viajeActivo.estado_viaje === 'aceptado' ? 'Unidad en camino' : 'Buscando unidad...'}
            </h2>
            <p className="text-zinc-500 text-[10px] mt-3 font-bold uppercase tracking-widest truncate">{viajeActivo.destino}</p>
            
            <button onClick={() => setShowChat(true)} className="w-full bg-white/5 border border-white/10 text-[#39FF14] py-4 rounded-2xl mt-8 font-black text-xs tracking-widest hover:bg-white/10 transition-all">
              CHAT CON CONDUCTOR 💬
            </button>
          </div>
        </div>
      ...
      {showChat && (
        <div className="fixed inset-0 bg-black z-[5000] p-6 flex flex-col animate-in slide-in-from-bottom duration-300">
          <div className="flex justify-between items-center mb-8">
            <span className="font-black italic text-xl tracking-tighter text-white">CHAT <span className="text-[#39FF14]">TXMD</span></span>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 no-scrollbar pb-4">
            {mensajes.map((m, i) => (
              <div key={i} className={`flex ${m.remitente === 'cliente' ? 'justify-end' : 'justify-start'}`}>
                <div className={`p-4 rounded-[20px] max-w-[85%] text-sm font-bold ${m.remitente === 'cliente' ? 'bg-[#39FF14] text-black shadow-lg shadow-[#39FF14]/10' : 'bg-zinc-900 text-white border border-white/5'}`}>
                  {m.contenido}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={enviarMensaje} className="mt-6 flex gap-2">
            <input 
              ...
              className="flex-1 bg-zinc-900 border border-white/10 p-4 rounded-2xl outline-none focus:border-[#39FF14] transition-all text-sm" 
            />
            <button type="submit" className="bg-[#39FF14] text-black px-6 rounded-2xl font-black uppercase text-[10px]">Enviar</button>
          </form>
        </div>
      )}
    </div>
